...
function get20s(arr){
  ...
  for(let i = 0; i<arr.length; i++){
    // maybe using regex isn't necessary since years like '190 - 199' isn't realistically possible
    if(arr[i].years.match(/19\d\d - 19\d\d/)!==null){
      res.push(arr[i].name);
    }
  }
  return res;
...
// image link via a keyword is not implemented, can't find a good way to do it fast without setting up some external API or scraping. I know it could be done though.
